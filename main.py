...
st.altair_chart(fake_news_daily, use_container_width=False)
...
st.altair_chart(daily_publi_per_domain, use_container_width=False)
...
# Define third graph - Top 10 articles per category
@st.cache(allow_output_mutation=True)
def graph3(df, df_top10):
	...
	top10_cat = alt.Chart(df_top10).mark_bar().encode(
...
top10_cat = graph3(df, df_top10)
st.altair_chart(top10_cat, use_container_width=False)
...
st.altair_chart(cat_per_origin, use_container_width=False)
...
def graph6(df_engagement_month_origin):
	month_slider = alt.binding_range(min=1, max=12, step=1)
	slider_selection_month = alt.selection_single(bind=month_slider, fields=['month_int'], name='slider')		click_cat_orig = alt.selection_multi(encodings=['color'])
	...
	engagement_per_origin = engagement_per_origin.add_selection(slider_selection_month)
		.transform_filter(slider_selection_month)
...
def graph7(df_engagement_month_category):
	...
	engagement_per_category = engagement_per_category.add_selection(slider_selection_month)
		.transform_filter(slider_selection_month)
